...
class JsonVelocityProcessorIntegerTest {
  ...
  @Test
  void empty() {
    JsonVelocityProcessor<BigInteger> processor = new JsonVelocityProcessorInteger(createVelocityDefinition("/velocity/no_path/a_value", null));
    assertThat(processor.valueFrom(jsonObject)).isEmpty();
  }

  @Test
  void emptyWithVarName() {
    JsonVelocityProcessor<BigInteger> processor = new JsonVelocityProcessorInteger(createVelocityDefinition("/velocity/no_path/a_value", "/velocity/i_am_a_identifier"));
    assertThat(processor.variableNameFrom(jsonObject)).isPresent().hasValue(VARIABLE_NAME);
  }

  @Test
  void emptyWithNoVarName() {
    JsonVelocityProcessor<BigInteger> processor = new JsonVelocityProcessorInteger(createVelocityDefinition("/velocity/no_path/a_value", "/velocity/no_exist"));
    assertThat(processor.variableNameFrom(jsonObject)).isEmpty();
  }

  @Test
  void valueFrom() {
    JsonVelocityProcessor<BigInteger> processor = new JsonVelocityProcessorInteger(createVelocityDefinition("/velocity/int_path/a_value", null));
  ...
  void valueFromString() {
    JsonVelocityProcessor<BigInteger> processor = new JsonVelocityProcessorInteger(createVelocityDefinition("/velocity/int_path/a_str_value", null));
    assertThat(processor.valueFrom(jsonObject))
        .isPresent()
        .hasValueSatisfying(v ->
            assertThat(v.value()).isEqualByComparingTo(BIG_INTEGER)
